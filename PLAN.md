...
- [x] **Vault earnings surprise was frozen**: `update_vault.py` forward-filled the last known `earnings_surprise`/`revenue_surprise` into every appended row, so no report after the incremental updater took over (2026-06-01) reached the vault/paper data. **Fixed**: `update_vault.py` now takes surprise from the FMP calendar (`latest_surprise`), and `scripts/refresh_vault_surprise.py` repaired the rows already written (133k of 209k post-freeze rows changed). Parity-check surprise match on vault events 74% → 82% (loop: 83%). Paper P&L since 2026-06 is recomputed on the corrected data at the next nightly run
- [ ] `roe`, `net_margin`, `debt_to_equity`, valuation ratios, `analyst_revision_3m`, `fcf_yield`, `dividend_yield` are still frozen in the vault since 2026-06-01 — they need per-ticker FMP calls (~11k for 2,700 tickers), which the free tier's 250/day can't cover. Needs a paid FMP plan or a bulk source
- [x] **Filing-date fundamentals** (`autoalpha/data/fundamentals.py`, `scripts/refresh_fundamentals.py`, weekly cron Sun 12:00 UTC): statements were joined on fiscal period end, ~30-45 days before filing. Rebuilt loop + vault point-in-time. Re-score 2026-09-24: combined book holdout alpha 21.1% → 2.0%/yr (t 0.17), pooled t 1.83 → 0.53; 0/4 pass
- [x] **PEAD through the gate** (`autoalpha/strategies/pead_et.py`, `scripts/evaluate_pead_gate.py`) — earnings-trader's rules on dataset columns: holdout alpha +9.5%/yr (t 0.62, 431 entries), forward −43.5%/yr (t −1.63, 70 entries), pooled t 0.15 → HOLD. Caveat: earnings-trader's thresholds were swept on 2022-24 backtests, which overlap the holdout
- [ ] `PEADStrategy` (strategies/pead.py) returns only new entries, but SimExecutor closes any held ticker absent from targets — it has been holding 1 day, not 10
- [x] `Runner._normalize_targets` always rescaled weights to sum to 1 (its docstring said "at most 1") and `run_paper.py` didn't normalize at all. **Fixed**: weights are only scaled down when they sum above 1 (the remainder is cash), and paper uses the same function. Re-score unchanged for the 4 active signals (they already sum to 1)
- [x] **Live PEAD on the autoalpha paper account** (`scripts/run_live_pead.py`, cron `31 13,14 * * 1-5` UTC → 9:31 ET): earnings-trader's 9:30 scan reproduced — exits/trailing stops, then entries on today's reactions as same-session market orders; `autoalpha/execution/risk.py` ports the kill switch (4% daily loss, 15% drawdown, `data/live/HALT`, latched until `resume()`), equity taken from the broker; broker is the system of record for holdings; refuses the real-money endpoint without `LIVE_TRADING_CONFIRMED=yes`. Daily Discord post shows both books side by side. Started 2026-09-25 on `PA3H1E1LFFIZ` ($100k, 10% per slot)
- [ ] After ~4 weeks side by side, compare entries/exits/fills with earnings-trader and decide whether to retire it (PEAD itself fails the gate: pooled t 0.15)
...
- [ ] Decide the long-term admission rule. Options: enforce real DSR (admits ~nothing at current N); argue N down legitimately by counting *independent* trials — clustering the book by return correlation, since 3,541 variations on "quality × revision" are plainly not independent; or keep PSR as a cheap screen and let the holdout + forward stages carry the burden permanently
  - **Decision 2026-08-27**: run the holdout + forward gate as the operative rule. It is the only option whose validity does not depend on a trial count that can no longer be reconstructed — return series exist for just 139 of 3,541 trials (the 4 active, 108 pruned, 27 retired); the 3,252 rejects were never stored, so the effective N of the search that produced this book is unrecoverable
- [x] **Persist return series for every trial** (`memory.store_trial_returns` / `all_trial_returns`, called from `loop.py` before any accept/reject branching). zlib-compressed JSON, ~20 KB per 1,600-bar trial (~68 MB per 3,500 trials). This makes effective-N clustering available for *future* admissions — it cannot be reconstructed retroactively, which is precisely why the current book cannot be judged that way. Backfilled the 139 hypotheses that had `portfolio_alpha` rows (2.1 MB)
...
**Goal (agreed 2026-09-25):**
- **Pass**: pooled holdout + forward alpha t ≥ 2.0 vs the equal-weight universe, both windows positive, net of 11 bps (`promotion_status.gate_verdict`, same as every other signal)
- **Economic bar**: net alpha ≥ 5%/yr, max drawdown < 20%, long-only top decile
- **Kill rule**: if the internal check (fit 2018–2021, trade 2022 → 2024-05 on loop data) and the holdout both show alpha t < 1.0, stop before paying for new data
- **No tuning on the holdout**: hyperparameters fixed in `ml_cross_section.py`; changes may be justified only by the internal check, so the gate windows stay untainted
...
- [ ] **Unfreeze fundamentals**: the FMP plan is Premium (750 calls/min), so the ~11k per-ticker calls blocking the frozen-fundamentals item above are no longer a constraint. Checked 2026-09-25: roe, valuation ratios and fcf_yield do update in the vault through 2026Q3 (filing-date rebuild); only `analyst_revision_3m` is still frozen (no changes since 2026Q2)
...
- [x] **Pilot on existing data (2026-09-25)** — `autoalpha/strategies/ml_cross_section.py` (sklearn `HistGradientBoostingRegressor` — LightGBM isn't installed; 25 ranked features, label = ranked next-21-bar open-to-open return, monthly top-decile equal weight, penny stocks excluded), `SimExecutor(hold_unchanged=True)` so held names drift between rebalances, `scripts/evaluate_ml_gate.py`, `tests/test_ml_cross_section.py`. Results vs an open-to-open equal-weight benchmark, net 11 bps: internal (fit 2018–21, trade 2022 → 2024-05) alpha −3.0%/yr t −0.85 beta 1.02; holdout −12.8%/yr t −2.94; forward −16.5%/yr t −1.70; pooled −13.8%/yr **t −3.37 → HOLD**. The book was fully invested (~250 names, gross ≈ 1.0, 20% vol) and returned ~8%/yr in the holdout vs ~21–26%/yr for the equal-weight universe (median stock 6.7%/yr — a skewed, small-cap-led rally). **Kill rule hit** (internal and holdout both t < 1.0): no paid data until something changes the picture. Hyperparameters were not touched after seeing the holdout
- [x] **Gate benchmark is misaligned with the fill model (found 2026-09-25)** — `SimExecutor` marks NAV at each bar's *open*, so strategy returns are open-to-open, while `evaluate_vault._benchmark_returns` (`scripts/evaluate_vault.py:63`), `run_paper.py:285` and `evaluate_pead_gate.run` build a *close-to-close* equal-weight benchmark on the same date label. The two overlap only overnight: correlation ~0.17, beta collapses to ~0, and the market's return is booked as alpha. This is why the book looked "close to market-neutral" (beta 0.07). Re-scoring `data/vault_results.json` holdout against an open-to-open benchmark: combined book +2.0%/yr t 0.17 beta 0.01 → **−13.6%/yr t −2.23 beta 0.69**; EV/EBITDA −6.3 → −18.9 (t −2.98), revision quality +7.6 → −7.3, quality dip +9.4 → −11.3, QARP −2.7 → −16.9. Fix: build the benchmark from `Open` in all three places (or mark NAV at the close), then re-run vault evaluation, paper and the PEAD gate. Changes every recorded gate number, so left for an explicit decision
  - **Fixed 2026-09-25**: `alpha.equal_weight_benchmark(opens)` (open-to-open, non-positive prices → missing) used by `evaluate_vault.py`, `run_paper.py`, `evaluate_pead_gate.py`, `evaluate_ml_gate.py`; `tests/test_benchmark.py`. Re-ran vault, paper, weights, promotion and both gate scripts. Combined book: holdout −15.1%/yr t −2.38, forward +18.9%/yr t 1.31, **pooled −10.5%/yr t −1.81**; per-signal pooled t: QARP −0.81, quality dip −1.18, revision quality −0.90, EV/EBITDA −2.71. PEAD: holdout −6.7% t −0.68, forward −51.3% t −2.08, pooled t −1.45. ML ranker unchanged at pooled t −3.37. 0 signals pass. Earlier "Status" lines in this file (e.g. book +23.1%/yr t 1.96, beta 0.07) were measured against the misaligned benchmark
- [x] FF5 residualization (`compute_alpha_returns`, used by `update_weights.py` and the paper report's FF5 line) has the same mismatch: Ken French factors are close-to-close, strategy returns are open-to-open. No open-to-open factor series exists; options are marking NAV at the close in `SimExecutor`, or regressing on same-day and lagged factors — **done 2026-09-25** with same-day + lagged factors (see round 2 below)
- [x] **Gate measurement fixes, round 2 (2026-09-25)**:
  - Benchmark price floor: `equal_weight_benchmark` counts a stock-day only if it opened ≥ $5 the bar before (`BENCHMARK_MIN_PRICE`, same floor the strategies use). Holdout EW return 25.7%/yr → 18.4%/yr; SPY open-to-open 19.7%/yr
  - FF5 timing: `_open_aligned` regresses open-to-open returns on same-day and previous-day factors (Dimson-style, betas summed); `tests/test_benchmark.py` checks a beta split across days is recovered
  - **Holdout stage used FF5 residuals, not returns**: `subprocess_runner` emits `alpha_series` (factor exposure already stripped) as `returns`, and `evaluate_vault` stored those as each signal's holdout `daily_returns` — so the gate regressed residuals on the benchmark while the forward stage used raw returns. Child now also emits `raw_returns`/`raw_return_dates`; `evaluate_vault` uses them
  - **Holdout clamp never reached the backtest**: `evaluate_vault` clamped its in-memory frame but passed the full vault path to the subprocess, so signal series ran to 2026-07-31 (549 bars vs 501), two months into the forward window. Output is now clamped to `holdout_end`
  - **Re-score with all fixes**: combined book holdout −2.4%/yr t −0.38, forward +23.5%/yr t 1.68, **pooled +1.2%/yr t 0.21**; per-signal pooled t: QARP 0.17, quality dip 0.51, revision quality 0.79, EV/EBITDA −1.22. PEAD pooled −8.9%/yr t −0.97 (forward −47.2%, t −1.94). ML ranker pooled −8.8%/yr t −2.29 (internal +0.6% t 0.19). Book vs SPY (open-to-open): holdout −8.1%/yr t −1.05, beta 0.39. **0 strategies pass; none is distinguishable from the benchmark**
- [x] `run_paper.py` skips `executor.execute` when a strategy returns `{}` (`if prev_targets and open_prices`), so held positions are never closed and no NAV is recorded that day — unlike `Runner`, which always executes. Harmless for today's always-invested signals; wrong for event strategies. **Fixed 2026-09-25** in `run_paper.py` and the same pattern in `Runner.run_live`; test in `tests/test_phase1.py`
- [x] **earnings-trader paper trades vs SPY (2026-09-25)**: 113 closed round trips in `earnings-trader/data/trades_log.jsonl` (2026-02-26 → 2026-09, all paper/sim), mean +0.03%/trade vs SPY +0.70% over the same holding windows → excess −0.67%/trade, t −0.61, win rate 47%, median hold 13 days. Consistent with the PEAD gate result: no evidence of edge
- [x] **Do published anomalies still pay after 2020? (2026-09-25)** — [Open Source Asset Pricing](https://www.openassetpricing.com/) long-short portfolio returns (Chen & Zimmermann, data through 2024-12), 165 "clear" predictors, mean monthly LS return averaged across predictors, gross of costs:

  | Period | Original-paper method | Price > $5 screen |
  |---|---|---|
  | In-sample | 0.75% | 0.62% |
  | Out-of-sample, pre-publication | 0.46% | 0.37% |
  | Post-publication to 2019 | 0.25% | 0.17% |
  | 2020–2024 | 0.42% | 0.30% |

  Equal-weight portfolio of all predictors' LS legs, 2020-01 → 2024-12: 0.45%/mo t 3.43 (original), **0.30%/mo t 2.23 ($5 screen)**. Almost all of it came in 2021–22 (~0.9%/mo); 2020 and 2023 were ≤ 0 with the $5 screen. So the anomaly zoo as a whole still earned a positive gross long-short premium post-2020 — but it needs a short leg (autoalpha and the paper account are long-only), and turnover costs are not deducted. Implication for autoalpha: a diversified multi-anomaly long-short composite is the one approach with post-2020 evidence; single hand-built long-only signals on 2.3 years of data are not testable to t = 2
- [x] **ETF trend following (2026-09-25)** — `autoalpha/strategies/etf_trend.py` (12-month trend, inverse 63-day vol weights, long-only/cash, monthly), `scripts/evaluate_etf_trend.py`, `tests/test_etf_trend.py`. Pre-registered: 10 ETFs (SPY IWM EFA EEM TLT IEF LQD GLD DBC VNQ), 11 bps, success = post-2013 alpha vs 60/40 t ≥ 2 and positive pre-2013 alpha. Results (open-to-open): pre-publication 2003-07 → 2012 Sharpe 1.25, maxDD −8%, alpha vs 60/40 +5.8%/yr t 3.31; **post-publication 2013 → 2026-09 Sharpe 0.65 vs 1.00 for 60/40, CAGR 3.4% vs 9.7%, alpha −0.2%/yr t −0.22 → FAIL**. It still halves drawdown (−14% vs −22%) at ~5.5% vol, but the edge the literature documented did not persist out of sample on this universe
- [ ] Decide whether to pursue a long-short multi-anomaly composite (needs short support in `SimExecutor`/`AlpacaExecutor`, a cost model for turnover, and the OSAP signal definitions rebuilt on our data), or stop signal research
- [x] Equal-weight benchmark is also inflated by bad price spikes (e.g. BYND +2,910% on 2026-08-13): daily-rebalanced open-to-open EW holdout return 25.7%/yr → 21.0%/yr with |daily return| ≥ 50% excluded. Consider winsorizing or requiring a price/liquidity floor in the benchmark
