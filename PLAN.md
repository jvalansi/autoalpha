...
- [ ] FF5 residualization (`compute_alpha_returns`, used by `update_weights.py` and the paper report's FF5 line) has the same mismatch: Ken French factors are close-to-close, strategy returns are open-to-open. No open-to-open factor series exists; options are marking NAV at the close in `SimExecutor`, or regressing on same-day and lagged factors
- [ ] Equal-weight benchmark is also inflated by bad price spikes (e.g. BYND +2,910% on 2026-08-13): daily-rebalanced open-to-open EW holdout return 25.7%/yr → 21.0%/yr with |daily return| ≥ 50% excluded. Consider winsorizing or requiring a price/liquidity floor in the benchmark
