...
- [ ] `run_paper.py` skips `executor.execute` when a strategy returns `{}` (`if prev_targets and open_prices`), so held positions are never closed and no NAV is recorded that day — unlike `Runner`, which always executes. Harmless for today's always-invested signals; wrong for event strategies
