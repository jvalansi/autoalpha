...
(2024-05-21 → today). Returns are computed using full history so
252-day lookbacks near the vault start are accurate.
...
def load_ohlcv_full(ticker: str) -> pd.DataFrame:
    """Load full OHLCV history (training + vault) so lookback returns are correct."""
    frames = []
...
def build_ticker(ticker: str, vix: pd.Series) -> pd.DataFrame:
    ...
    # Join on full index so forward-fill is correct near vault boundary
    ...
    df = pd.concat([full, ret, fund, earn, vix_aligned], axis=1)
    for col in NAN_COLS:
        df[col] = np.nan
...
def main() -> None:
    ...
    # Fetch VIX for full period (need it aligned with full OHLCV index)
    start = "2018-01-01"
    end = VAULT_END.strftime("%Y-%m-%d")
